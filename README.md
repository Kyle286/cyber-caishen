...
消费分析（流式输出 · Flash/Pro 模型切换 · 冲动指数 · 机会成本 · 6 步推理链）：
...
- **流式输出**：回复采用 SSE 流式打字机呈现；结构化分析卡片（冲动指数/比价/CoT 等）随首个事件**秒级先出**，再逐字流出文案，体验顺滑。
- **模型可切换**：顶部一键在 `deepseek-v4-flash`（响应快·默认）与 `deepseek-v4-pro`（推理更强）之间切换，按需权衡速度与质量。
- **双人格角色扮演**：赛博财神（温暖鼓励·理性引导）/ 毒舌闺蜜（犀利劝退·拒绝智商税），一键切换；System Prompt 含 few-shot 示例与输出约束，切换人格即重置会话，绝不串味。
- **规则优先 + LLM 兜底的意图识别**：基于品类词典的物品识别（避免抓到价格数字/助词等垃圾），支持消费 / 设目标 / 查进度 / **克制(resist)** / 闲聊；处理**否定**（"不想买了"）、**纯询价**（"5000的相机贵吗"）、**多金额语义角色**（"月薪3000想买2万的包"取 2 万）。**规则不确定时（如未收录品类）自动 fallback 到 LLM 抽取槽位**，兼顾速度与覆盖面。
...
| POST | `/api/chat` | 核心对话（非流式），返回人格化回复 + 结构化分析 + 冲动指数 + CoT（支持多轮 history/context、model 选择） |
| POST | `/api/chat/stream` | 同上，**SSE 流式**：先发 `meta`（结构化分析）再逐段发 `delta`（回复增量），末尾 `done` |
